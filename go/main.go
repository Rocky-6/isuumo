...
var chairDBConnectionData *MySQLConnectionEnv
...
type Estate struct {
	...
	Point          []byte  `db:"point" json:"point"`
...
func NewChairDBConnectionEnv() *MySQLConnectionEnv {
	return &MySQLConnectionEnv{
		...
		Port:     getEnv("CHAIR_DB_PORT", "3306"),
...
func NewEstateDBConnectionEnv() *MySQLConnectionEnv {
	return &MySQLConnectionEnv{
		Host:     getEnv("MYSQL_HOST", "127.0.0.1"),
		Port:     getEnv("ESTATE_DB_PORT", "3307"),
		User:     getEnv("MYSQL_USER", "isucon"),
		DBName:   getEnv("MYSQL_DBNAME", "isuumo"),
		Password: getEnv("MYSQL_PASS", "isucon"),
	}
...
func main() {
	...
	// chair DB
	chairDBConnectionData = NewChairDBConnectionEnv()
	var err error
	...
	// estate DB
	estateDBConnectionData = NewEstateDBConnectionEnv()
...
func initialize(c echo.Context) error {
	...
	paths := []string{
		...
		filepath.Join(sqlDir, "3_AlterSchema.sql"),
	}

	for _, p := range paths {
		sqlFile, _ := filepath.Abs(p)
		chairCmdStr := fmt.Sprintf("mysql -h %v -u %v -p%v -P %v %v < %v",
			chairDBConnectionData.Host,
			chairDBConnectionData.User,
			chairDBConnectionData.Password,
			chairDBConnectionData.Port,
			chairDBConnectionData.DBName,
			sqlFile,
		)
		...
		estateCmdStr := fmt.Sprintf("mysql -h %v -u %v -p%v -P %v %v < %v",
		...
		if err := exec.Command("bash", "-c", estateCmdStr).Run(); err != nil {
			c.Logger().Errorf("Initialize script error : %v", err)
			return c.NoContent(http.StatusInternalServerError)
		}
	}
...
func postChair(c echo.Context) error {
	...
	stmtStr := query + strings.Join(placeholders, ",")
	_, err = tx.Exec(stmtStr, values...)
	if err != nil {
		c.Logger().Errorf("failed to execute bulk insert: %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
